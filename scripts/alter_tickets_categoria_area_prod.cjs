// Agrega columnas categoria y area a tickets en PROD (DigitalOcean).
// Solo se ejecuta si DATABASE_URL apunta a ondigitalocean. Cambios aditivos:
//   - categoria VARCHAR(255) NULL
//   - area VARCHAR(50) NOT NULL DEFAULT 'QEB'
//   - INDEX idx_tickets_area
// La app anterior (sin este cambio de codigo) sigue funcionando: ignora ambas
// columnas. La app nueva empieza a usarlas al hacer deploy.
//
// Uso:
//   node scripts/alter_tickets_categoria_area_prod.cjs           # dry-run
//   node scripts/alter_tickets_categoria_area_prod.cjs --commit  # aplica
//
// IMPORTANTE: apuntar DATABASE_URL a prod (DigitalOcean) antes de correrlo.
// Ej: DATABASE_URL="mysql://..." node scripts/alter_tickets_categoria_area_prod.cjs
...
async function main() {
  ...
  if (!host.includes('ondigitalocean')) {
    console.error('SEGURIDAD: este script es solo para PROD (DigitalOcean).');
    console.error(`Host actual: ${host}. Aborta.`);
    process.exit(1);
  }
  ...
  console.log(`Conectado a ${host} db=${database} (PROD)`);
  ...
  console.log(`\nTickets existentes en PROD: ${Number(count[0].c)} (quedan area='QEB', categoria=NULL)`);
