...
export class PropuestasController {
  async getAll(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      if (search) {
        whereConditions += ` AND (CAST(pr.id AS CHAR) LIKE ? OR pr.articulo LIKE ? OR pr.descripcion LIKE ? OR pr.asignado LIKE ? OR cl.T1_U_Cliente LIKE ? OR cl.T0_U_RazonSocial LIKE ? OR cl.T2_U_Marca LIKE ?)`;
