    ...
    try {
      ...
      if (
        ...
        if (todasFinalizadas) {
          ...
          // Notificar al creador al cambiar el status (para Ajuste CTO y Ajuste de Caras)
...
// Obtener lista de usuarios para asignación (filtrado por equipo)
  ...
  async createReservas(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const scData = solicitudCaraId ? await prisma.solicitudCaras.findUnique({
        where: { id: solicitudCaraId },
        select: { articulo: true },
      }) : null;
  ...
  async deleteReservas(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const reservasInfo = await prisma.reservas.findMany({
        where: { id: { in: reservaIds } },
        select: { id: true, solicitudCaras_id: true, inventario_id: true, estatus: true, APS: true },
      });
  ...
  async bulkUpdateCaras(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const firstCara = await prisma.solicitudCaras.findUnique({
        where: { id: parseInt(carasToUpdate[0].caraId) },
        select: { idquote: true }
      });
      const idquote = firstCara?.idquote || '';
      ...
      console.error('Error in campanas.bulkUpdateCaras:', error);
