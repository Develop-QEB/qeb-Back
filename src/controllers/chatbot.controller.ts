...
2. Solicitudes - Los ejecutivos crean solicitudes de espacios publicitarios. Click en Nueva Solicitud, llenar cliente, caras, fechas. Estados: Pendiente, En revision, Aprobada, Rechazada.
   - Periodo: se selecciona en "Catorcenas" o "Mensual". Una catorcena es un periodo de aproximadamente 15 dias (hay 26 catorcenas en el año, numeradas del 1 al 26). Mensual divide el año en 12 meses. La eleccion define como se cobran y programan los espacios.
   - Campos de una cara: Renta (numero de caras que el cliente paga), Bonificacion (numero de caras que se dan gratis al cliente como parte del trato, NO es descuento en dinero sino cantidad de inventario adicional sin costo), Tarifa Publica (numero de caras a precio de lista). Todos estos campos son CANTIDADES de inventario, no montos de dinero.
3. Propuestas - Se generan de solicitudes aprobadas. Se asigna inventario con mapa interactivo. Se comparte con cliente via link publico.
   - Tipos de espacios: Flujo (espectacular orientado a favor del trafico, una cara), Contraflujo (orientado en contra del trafico, la cara opuesta), Completo (parabuses o estructura que tiene AMBAS caras: flujo + contraflujo juntas, se reservan como unidad). Un espacio "Completo" cuenta como 2 caras.
   - Al asignar inventario hay un boton "Agrupar" (icono de regla) que agrupa los espacios disponibles por cercania geografica para facilitar la seleccion. Modos: "Distancia" (usa coordenadas GPS reales con algoritmo haversine, con selector de radio: 100m, 200m, 500m, 1km, 1.5km, 2km, 3km) o "Listado" (agrupa secuencialmente por posicion en la lista). Campo de tamano de grupo: cuantos espacios maximo por grupo (default 10). Al activarse aparece una columna "Grupo" en la tabla.
